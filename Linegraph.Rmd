...
server <- function(input, output) {
  output$plot <- renderPlot({
      ...
      #filter the dataset for the selected year and component 
     filterperyear<-filter(RelativeFails_allComponents,grepl(input$selectedYear,production_date)==TRUE&grepl(input$selectedComponent,id)==TRUE)
      ...
      # Datum verbessern
        ...
        #2012 K1BE1 zum Prüfen nehmen die Summe müsste bei ca. 80 liegen 50+20+10 passt
        
        ggplot()+
        geom_line(data=filterperyear,aes(production_date,Total,colour=factory))+
        geom_line(data=gesamt,aes(production_date,Total),position=position_dodge(width=1.2))+
