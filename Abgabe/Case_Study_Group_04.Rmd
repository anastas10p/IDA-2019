...
```{r import, eval = FALSE}
# Create a named list for the component tibbles.
...
# We read the data using the proper delimiters and any necessary changes, since not all files could be read using only read.delim
...
# K1DI2 needed to go through some changes.
# We read it as a strin
...
# We clear unnecessary whitespace and characters.
...
# We substitue unnecessary characters and put newline characters where needed.
...
save(errors_by_factory, errors_by_id, errors_by_id_week, file = "Final_Data_Group_04.RData")
```
...
```{r data, echo = TRUE}
load("Final_Data_Group_04.RData")
```
...
ui <- fluidPage(
  ...
  titlePanel(title = div(
    "Error Statistics: Factories and Components",
    img(
